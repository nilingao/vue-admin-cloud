const FSRTCClient = (function (exports) {
	'use strict';
	...
	function extractVersion(uastring, expr, pos) {
	  ...
	  return match && match.length >= pos && Number.parseInt(match[pos], 10);
	}
	...
	function wrapPeerConnectionEvent(window, eventNameToWrap, wrapper) {
	  ...
	  proto.addEventListener = function(nativeEventName, cb) {
	    if (nativeEventName !== eventNameToWrap) {
	      return Reflect.apply(nativeAddEventListener, this, arguments);
	    }
	    ...
	    return Reflect.apply(nativeAddEventListener, this, [nativeEventName,
	      wrappedCallback]);
	  };
	  ...
	  proto.removeEventListener = function(nativeEventName, cb) {
	    if (nativeEventName !== eventNameToWrap || !this._eventMap
	      ...
	      return Reflect.apply(nativeRemoveEventListener, this, arguments);
	    }
	    ...
	    return Reflect.apply(nativeRemoveEventListener, this, [nativeEventName,
	      unwrappedCb]);
	  };
	...
	function detectBrowser(window) {
	  ...
	  if (navigator.mozGetUserMedia) { // Firefox.
	    ...
	    result.version = extractVersion(navigator.userAgent,
	      ...
	      /Edge\/(\d+).(\d+)$/.test(navigator.userAgent)) { // Edge.
	    result.browser = 'edge';
	    result.version = extractVersion(navigator.userAgent,
	        /Edge\/(\d+).(\d+)$/, 2);
	  } else if (window.RTCPeerConnection &&
	      /AppleWebKit\/(\d+)\./.test(navigator.userAgent)) { // Safari.
	...
	function compactObject(data) {
	  ...
	  return Object.keys(data).reduce((accumulator, key) => {
	    ...
	    const isEmptyObject = isObj && Object.keys(value).length === 0;
	      ...
	      if ((face && (face.exact === 'user' || face.exact === 'environment' ||
	        ...
	        if (matches) {
	          ...
	          .then(devices => {
	            ...
	            if (!dev && devices.length > 0 && matches.includes('back')) {
	  ...
	  if (navigator.mediaDevices.getUserMedia) {
	    ...
	    navigator.mediaDevices.getUserMedia = function(cs) {
	      return shimConstraints_(cs, c => origGetUserMedia(c).then(stream => {
	        if (c.audio && stream.getAudioTracks().length === 0 ||
	            c.video && stream.getVideoTracks().length === 0) {
	        ...
	        return stream;
	      }, error_ => { throw shimError_(error_); }));
	    };
	  }
	}
	  ...
	  if (typeof window === 'object' && window.RTCPeerConnection && !('ontrack' in
	    ...
	    window.RTCPeerConnection.prototype.setRemoteDescription =
	      function setRemoteDescription() {
	        if (!this._ontrackpoly) {
	          this._ontrackpoly = (e) => {
	            ...
	            e.stream.addEventListener('addtrack', te => {
	              let receiver;
	              receiver = window.RTCPeerConnection.prototype.getReceivers ? this.getReceivers()
	                  .find(r => r.track && r.track.id === te.track.id) : {track: te.track};
	            ...
	            e.stream.getTracks().forEach(track => {
	              let receiver;
	              receiver = window.RTCPeerConnection.prototype.getReceivers ? this.getReceivers()
	                  .find(r => r.track && r.track.id === track.id) : {track};
	              const event = new Event('track');
	              event.track = track;
	              event.receiver = receiver;
	              event.transceiver = {receiver};
	        ...
	        return Reflect.apply(origSetRemoteDescription, this, arguments);
	      };
	  } else {
	  ...
	  if (typeof window === 'object' && window.RTCPeerConnection &&
	      ...
	      return {
	        ...
	        get dtmf() {
	          if (this._dtmf === undefined) {
	            this._dtmf = track.kind === 'audio' ? pc.createDTMFSender(track) : null;
	          }
	          return this._dtmf;
	        },
	        _pc: pc
	    ...
	    if (!window.RTCPeerConnection.prototype.getSenders) {
	      window.RTCPeerConnection.prototype.getSenders = function getSenders() {
	        ...
	        return [...this._senders]; // return a copy of the internal state.
	      };
	      const origAddTrack = window.RTCPeerConnection.prototype.addTrack;
	      window.RTCPeerConnection.prototype.addTrack =
	        function addTrack(track, stream) {
	          let sender = Reflect.apply(origAddTrack, this, arguments);
	          if (!sender) {
	      ...
	      window.RTCPeerConnection.prototype.removeTrack =
	        function removeTrack(sender) {
	          Reflect.apply(origRemoveTrack, this, arguments);
	    ...
	    window.RTCPeerConnection.prototype.addStream = function addStream(stream) {
	      this._senders = this._senders || [];
	      Reflect.apply(origAddStream, this, [stream]);
	      stream.getTracks().forEach(track => {
	    ...
	    window.RTCPeerConnection.prototype.removeStream =
	      function removeStream(stream) {
	        this._senders = this._senders || [];
	        Reflect.apply(origRemoveStream, this, [stream]);
	    ...
	    window.RTCPeerConnection.prototype.getSenders = function getSenders() {
	      const senders = Reflect.apply(origGetSenders, this, []);
	    ...
	    Object.defineProperty(window.RTCRtpSender.prototype, 'dtmf', {
	      get() {
	        if (this._dtmf === undefined) {
	          this._dtmf = this.track.kind === 'audio' ? this._pc.createDTMFSender(this.track) : null;
	        }
	        return this._dtmf;
	      }
	    });
	...
	function shimGetStats(window) {
	  ...
	  window.RTCPeerConnection.prototype.getStats = function getStats() {
	    ...
	    if (arguments.length > 0 && typeof selector === 'function') {
	      return Reflect.apply(origGetStats, this, arguments);
	    }
	    ...
	    if (origGetStats.length === 0 && (arguments.length === 0 ||
	      ...
	      return Reflect.apply(origGetStats, this, []);
	    }
	    ...
	    if (arguments.length >= 2) {
	      ...
	      return Reflect.apply(origGetStats, this, [successCallbackWrapper_,
	        selector]);
	    }

	    // promise-support
	    return new Promise((resolve, reject) => {
	      Reflect.apply(origGetStats, this, [
	        function(response) {
	  ...
	  if (!('getStats' in window.RTCRtpSender.prototype)) {
	    ...
	    if (origAddTrack) {
	      window.RTCPeerConnection.prototype.addTrack = function addTrack() {
	        const sender = Reflect.apply(origAddTrack, this, arguments);
	        sender._pc = this;
	        return sender;
	      };
	    }
	    ...
	    if (origGetReceivers) {
	      window.RTCPeerConnection.prototype.getReceivers =
	        function getReceivers() {
	          const receivers = Reflect.apply(origGetReceivers, this, []);
	...
	function shimAddTrackRemoveTrackWithNative(window) {
	  ...
	  window.RTCPeerConnection.prototype.addTrack =
	    function addTrack(track, stream) {
	      if (!stream) {
	        return Reflect.apply(origAddTrack, this, arguments);
	      }
	      this._shimmedLocalStreams = this._shimmedLocalStreams || {};
	  ...
	  window.RTCPeerConnection.prototype.addStream = function addStream(stream) {
	    ...
	    Reflect.apply(origAddStream, this, arguments);
	    const newSenders = this.getSenders()
	      .filter(newSender => !existingSenders.includes(newSender));
	  ...
	  window.RTCPeerConnection.prototype.removeStream =
	    function removeStream(stream) {
	      ...
	      return Reflect.apply(origRemoveStream, this, arguments);
	    };
	  ...
	  window.RTCPeerConnection.prototype.removeTrack =
	    function removeTrack(sender) {
	      ...
	      return Reflect.apply(origRemoveTrack, this, arguments);
	    };
	}
	...
	function shimAddTrackRemoveTrack(window, browserDetails) {
	  ...
	  window.RTCPeerConnection.prototype.removeStream =
	    function removeStream(stream) {
	      ...
	      Reflect.apply(origRemoveStream, this, [(this._streams[stream.id] || stream)]);
	  ...
	  function replaceInternalStreamId(pc, description) {
	    ...
	    Object.keys(pc._reverseStreams || []).forEach(internalId => {
	      ...
	      sdp = sdp.replaceAll(new RegExp(internalStream.id, 'g'),
	          externalStream.id);
	    });
	  ...
	  function replaceExternalStreamId(pc, description) {
	    ...
	    Object.keys(pc._reverseStreams || []).forEach(internalId => {
	      const externalStream = pc._reverseStreams[internalId];
	      const internalStream = pc._streams[externalStream.id];
	      sdp = sdp.replaceAll(new RegExp(externalStream.id, 'g'),
	          internalStream.id);
	    });
	    return new RTCSessionDescription({
	      type: description.type,
	      ...
	      if (isLegacyCall) {
	        return Reflect.apply(nativeMethod, this, [
	          (description) => {
	            ...
	            Reflect.apply(args[0], null, [desc]);
	          },
	          (err) => {
	            if (args[1]) {
	  ...
	  window.RTCPeerConnection.prototype.setLocalDescription =
	    function setLocalDescription() {
	      if (arguments.length === 0 || !arguments[0].type) {
	        return Reflect.apply(origSetLocalDescription, this, arguments);
	      }
	...
	function fixNegotiationNeeded(window, browserDetails) {
	  wrapPeerConnectionEvent(window, 'negotiationneeded', e => {
	    ...
	    if ((browserDetails.version < 72 || (pc.getConfiguration &&
	        pc.getConfiguration().sdpSemantics === 'plan-b')) && pc.signalingState !== 'stable') {
	        return;
	      }
	    return e;
	  });
	...
	const commonjsGlobal = typeof globalThis === 'undefined' ? typeof window === 'undefined' ? typeof global === 'undefined' ? typeof self === 'undefined' ? {} : self : global : window : globalThis;
	...
	const sdp = createCommonjsModule((module) => {

	// SDP helpers.
	const SDPUtils = { generateIdentifier() {
	...
	SDPUtils.matchPrefix = function(blob, prefix) {
	  return SDPUtils.splitLines(blob).filter((line) => {
	...
	SDPUtils.parseCandidate = function(line) {
	  let parts;
	  // Parse both variants.
	  parts = line.indexOf('a=candidate:') === 0 ? line.slice(12).split(' ') : line.slice(10).split(' ');

	  const candidate = {
	    foundation: parts[0],
	    component: Number.parseInt(parts[1], 10),
	    protocol: parts[2].toLowerCase(),
	    priority: Number.parseInt(parts[3], 10),
	    ip: parts[4],
	    address: parts[4], // address is an alias for ip.
	    port: Number.parseInt(parts[5], 10),
	    // skip parts[6] == 'typ'
	    type: parts[7]
	  };
	  ...
	  for (let i = 8; i < parts.length; i += 2) {
	    switch (parts[i]) {
	      ...
	      case 'rport': {
	        candidate.relatedPort = Number.parseInt(parts[i + 1], 10);
	        break;
	      }
	      case 'tcptype': {
	...
	SDPUtils.writeCandidate = function(candidate) {
	  const sdp = [ candidate.foundation, candidate.component];
	  sdp.push(candidate.protocol.toUpperCase(), candidate.priority, candidate.address || candidate.ip, candidate.port);

	  const type = candidate.type;
	  sdp.push('typ', type);
	  if (type !== 'host' && candidate.relatedAddress &&
	      candidate.relatedPort) {
	    sdp.push('raddr', candidate.relatedAddress, 'rport', candidate.relatedPort);
	  }
	  if (candidate.tcpType && candidate.protocol.toLowerCase() === 'tcp') {
	    sdp.push('tcptype', candidate.tcpType);
	  }
	  if (candidate.usernameFragment || candidate.ufrag) {
	    sdp.push('ufrag', candidate.usernameFragment || candidate.ufrag);
	  }
	...
	SDPUtils.parseIceOptions = function(line) {
	  return line.slice(14).split(' ');
	};

	// Parses an rtpmap line, returns RTCRtpCoddecParameters. Sample input:
	// a=rtpmap:111 opus/48000/2
	SDPUtils.parseRtpMap = function(line) {
	  let parts = line.slice(9).split(' ');
	  const parsed = {
	    payloadType: Number.parseInt(parts.shift(), 10) // was: id
	  };

	  parts = parts[0].split('/');

	  parsed.name = parts[0];
	  parsed.clockRate = Number.parseInt(parts[1], 10); // was: clockrate
	  parsed.channels = parts.length === 3 ? Number.parseInt(parts[2], 10) : 1;
	...
	SDPUtils.writeRtcpFb = function(codec) {
	  ...
	  if (codec.rtcpFeedback && codec.rtcpFeedback.length > 0) {
	    ...
	    codec.rtcpFeedback.forEach((fb) => {
	      ...
	      }${fb.parameter && fb.parameter.length > 0 ? ` ${  fb.parameter}` : '' 
	          }\r\n`;
	    });
	  }
	  return lines;
	...
	SDPUtils.parseSsrcMedia = function(line) {
	  ...
	  const parts = {
	    ssrc: Number.parseInt(line.substr(7, sp - 7), 10)
	  };
	  const colon = line.indexOf(':', sp);
	  if (colon === -1) {
	    parts.attribute = line.slice(sp + 1);
	  } else {
	    parts.attribute = line.substr(sp + 1, colon - sp - 1);
	    parts.value = line.slice(colon + 1);
	  }
	  return parts;
	};
	...
	SDPUtils.parseSsrcGroup = function(line) {
	  ...
	  return {
	    ...
	    ssrcs: parts.map((ssrc) => {
	      return Number.parseInt(ssrc, 10);
	    })
	  };
	};
	...
	SDPUtils.parseCryptoLine = function(line) {
	  const parts = line.slice(9).split(' ');
	  return {
	    tag: Number.parseInt(parts[0], 10),
	    cryptoSuite: parts[1],
	    keyParams: parts[2],
	...
	SDPUtils.parseCryptoKeyParams = function(keyParams) {
	  ...
	  const parts = keyParams.slice(7).split('|');
	  return {
	    keyMethod: 'inline',
	    keySalt: parts[0],
	    lifeTime: parts[1],
	...
	SDPUtils.getIceParameters = function(mediaSection, sessionpart) {
	  ...
	  return {
	    usernameFragment: ufrag.slice(12),
	    password: pwd.slice(10),
	  };
	};
	...
	SDPUtils.parseRtpParameters = function(mediaSection) {
	  ...
	  for (let i = 3; i < mline.length; i++) { // find all codecs from mline[3..]
	    ...
	    if (rtpmapline) {
	      ...
	      codec.parameters = fmtps.length > 0 ? SDPUtils.parseFmtp(fmtps[0]) : {};
	...
	SDPUtils.parseRtpEncodingParameters = function(mediaSection) {
	  ...
	  description.codecs.forEach((codec) => {
	    if (codec.name.toUpperCase() === 'RTX' && codec.parameters.apt) {
	      let encParam = {
	        ssrc: primarySsrc,
	        codecPayloadType: Number.parseInt(codec.parameters.apt, 10)
	      };
	  ...
	  if (bandwidth.length > 0) {
	    if (bandwidth[0].indexOf('b=TIAS:') === 0) {
	      bandwidth = Number.parseInt(bandwidth[0].slice(7), 10);
	    } else if (bandwidth[0].indexOf('b=AS:') === 0) {
	      // use formula from JSEP to convert b=AS to TIAS value.
	      bandwidth = Number.parseInt(bandwidth[0].slice(5), 10) * 1000 * 0.95
	          - (50 * 40 * 8);
	    } else {
	      bandwidth = undefined;
	    }
	...
	SDPUtils.parseSctpDescription = function(mediaSection) {
	  ...
	  if (maxSizeLine.length > 0) {
	    maxMessageSize = Number.parseInt(maxSizeLine[0].slice(19), 10);
	  }
	  ...
	  if (sctpPort.length > 0) {
	    return {
	      port: Number.parseInt(sctpPort[0].slice(12), 10),
	      protocol: mline.fmt,
	      maxMessageSize
	    };
	  }
	  ...
	  if (sctpMapLines.length > 0) {
	    const parts = SDPUtils.matchPrefix(mediaSection, 'a=sctpmap:')[0]
	      ...
	      .split(' ');
	    return {
	      port: Number.parseInt(parts[0], 10),
	      protocol: parts[1],
	      maxMessageSize
	    };
	  }
	...
	SDPUtils.writeSctpDescription = function(media, sctp) {
	  let output = [];
	  output = media.protocol === 'DTLS/SCTP' ? [
	...
	SDPUtils.generateSessionId = function() {
	  return Math.random().toString().slice(2, 23);
	};
	...
	SDPUtils.writeSessionBoilerplate = function(sessId, sessVer, sessUser) {
	  let sessionId;
	  const version = sessVer === undefined ? 2 : sessVer;
	  sessionId = sessId ? sessId : SDPUtils.generateSessionId();
	...
	SDPUtils.parseOLine = function(mediaSection) {
	  ...
	  return {
	    username: parts[0],
	    sessionId: parts[1],
	    sessionVersion: Number.parseInt(parts[2], 10),
	    netType: parts[3],
	    addressType: parts[4],
	    address: parts[5]
	  };
	...
	function writeMediaSection(transceiver, caps, type, stream, dtlsRole) {
	  let sdp$1 = sdp.writeRtpDescription(transceiver.kind, caps);
	  ...
	  return iceServers.filter((server) => {
	    if (server && (server.urls || server.url)) {
	      ...
	      urls = urls.filter((url) => {
	        ...
	        return url.indexOf('stun:') === 0 && edgeVersion >= 14_393 &&
	            !url.includes('?transport=udp');
	      });
	    ...
	    for (let i = 0; i < remoteCapabilities.codecs.length; i++) {
	      ...
	      if (lCodec.name.toLowerCase() === rCodec.name.toLowerCase() &&
	        ...
	        if (lCodec.name.toLowerCase() === 'rtx' &&
	            lCodec.parameters && rCodec.parameters.apt && // for RTX we need to find the local rtx that has a apt
	          // which points to the same local codec as the remote one.
	          !rtxCapabilityMatches(lCodec, rCodec,
	...
	const rtcpeerconnection = function(window, edgeVersion) {
	  ...
	  RTCPeerConnection.prototype.addTrack = function(track, stream) {
	    ...
	    const alreadyExists = this.transceivers.find((s) => {
	  ...
	  RTCPeerConnection.prototype.removeTrack = function(sender) {
	    ...
	    const localStreams = this.transceivers.map((t) => {
	      return t.stream;
	    });
	  ...
	  RTCPeerConnection.prototype.getSenders = function() {
	    return this.transceivers.filter((transceiver) => {
	      return !!transceiver.rtpSender;
	    })
	    .map((transceiver) => {
	      return transceiver.rtpSender;
	  ...
	  RTCPeerConnection.prototype._createIceGatherer = function(sdpMLineIndex,
	    ...
	    } else if (this._iceGatherers.length > 0) {
	  ...
	  RTCPeerConnection.prototype._gather = function(mid, sdpMLineIndex) {
	    ...
	    iceGatherer.onlocalcandidate = function(evt) {
	      ...
	      if (end) {
	        ...
	        const serializedCandidate = sdp.writeCandidate(cand);
	  ...
	  RTCPeerConnection.prototype._transceive = function(transceiver,
	    ...
	    if (send && transceiver.rtpSender) {
	      ...
	      if (transceiver.recvEncodingParameters.length > 0) {
	    ...
	    if (recv && transceiver.rtpReceiver && params.codecs.length > 0) {
	      ...
	      params.encodings = transceiver.recvEncodingParameters.length > 0 ? transceiver.recvEncodingParameters : [{}];
	      params.rtcp = {
	        compound: transceiver.rtcpParameters.compound
	      };
	      ...
	      if (transceiver.sendEncodingParameters.length > 0) {
	  ...
	  RTCPeerConnection.prototype.setLocalDescription = function(description) {
	    ...
	    if (!['answer', 'offer'].includes(description.type)) {
	    ...
	    if (description.type === 'offer') {
	      ...
	      sections.forEach((mediaSection, sdpMLineIndex) => {
	        const caps = sdp.parseRtpParameters(mediaSection);
	      ...
	      sessionpart = sections.shift();
	      const isIceLite = sdp.matchPrefix(sessionpart,
	          'a=ice-lite').length > 0;
	      sections.forEach((mediaSection, sdpMLineIndex) => {
	        ...
	        const rejected = sdp.isRejected(mediaSection) &&
	            sdp.matchPrefix(mediaSection, 'a=bundle-only').length === 0;

	        if (!rejected && !transceiver.rejected) {
	          const remoteIceParameters = sdp.getIceParameters(
	              mediaSection, sessionpart);
	          const remoteDtlsParameters = sdp.getDtlsParameters(
	              mediaSection, sessionpart);
	          if (isIceLite) {
	  ...
	  RTCPeerConnection.prototype.setRemoteDescription = function(description) {
	    ...
	    const isIceLite = sdp.matchPrefix(sessionpart,
	        'a=ice-lite').length > 0;
	    const usingBundle = sdp.matchPrefix(sessionpart,
	        'a=group:BUNDLE ').length > 0;
	    pc.usingBundle = usingBundle;
	    const iceOptions = sdp.matchPrefix(sessionpart,
	        'a=ice-options:')[0];
	    pc.canTrickleIceCandidates = iceOptions ? iceOptions.slice(14).split(' ')
	          .includes('trickle') : false;

	    sections.forEach((mediaSection, sdpMLineIndex) => {
	      const lines = sdp.splitLines(mediaSection);
	      const kind = sdp.getKind(mediaSection);
	      // treat bundle-only as not-rejected.
	      const rejected = sdp.isRejected(mediaSection) &&
	          sdp.matchPrefix(mediaSection, 'a=bundle-only').length === 0;
	      const protocol = lines[0].slice(2).split(' ')[2];

	      const direction = sdp.getDirection(mediaSection, sessionpart);
	      const remoteMsid = sdp.parseMsid(mediaSection);

	      const mid = sdp.getMid(mediaSection) || sdp.generateIdentifier();
	      ...
	      const remoteCapabilities = sdp.parseRtpParameters(mediaSection);
	      ...
	      const rtcpParameters = sdp.parseRtcpParameters(mediaSection);

	      const isComplete = sdp.matchPrefix(mediaSection,
	          'a=end-of-candidates', sessionpart).length > 0;
	      const cands = sdp.matchPrefix(mediaSection, 'a=candidate:')
	          .map((cand) => {
	      ...
	      if (description.type === 'offer' && !rejected) {
	        ...
	        if (cands.length > 0 && transceiver.iceTransport.state === 'new') {
	        ...
	        if (direction === 'sendrecv' || direction === 'sendonly') {
	          ...
	          transceiver.associatedRemoteMediaStreams.forEach((s) => {
	            const nativeTrack = s.getTracks().find((t) => {
	        ...
	        if (cands.length > 0 && iceTransport.state === 'new') {
	        ...
	        const hasRtx = commonCapabilities.codecs.filter((c) => {
	  ...
	  RTCPeerConnection.prototype.createOffer = function() {
	    ...
	    let numAudioTracks = pc.transceivers.filter((t) => {
	      return t.kind === 'audio';
	    }).length;
	    let numVideoTracks = pc.transceivers.filter((t) => {
	      return t.kind === 'video';
	    }).length;
	    ...
	    let sdp$1 = sdp.writeSessionBoilerplate(pc._sdpSessionId,
	    ...
	    pc.transceivers.forEach((transceiver, sdpMLineIndex) => {
	      ...
	      const mid = transceiver.mid || sdp.generateIdentifier();
	      transceiver.mid = mid;
	    ...
	    if (pc._config.bundlePolicy !== 'max-compat') {
	      sdp$1 += `a=group:BUNDLE ${  pc.transceivers.map((t) => {
	        return t.mid;
	      }).join(' ')  }\r\n`;
	    }
	  ...
	  RTCPeerConnection.prototype.createAnswer = function() {
	    ...
	    const mediaSectionsInOffer = sdp.getMediaSections(
	    ...
	    pc.transceivers.forEach((transceiver, sdpMLineIndex) => {
	      ...
	      if (transceiver.stream) {
	        ...
	        if (localTrack && // add RTX
	          edgeVersion >= 15_019 && transceiver.kind === 'video' &&
	      ...
	      if (!pc._remoteDescription) {
	        ...
	        if (transceiver) {
	          ...
	          if ((sdpMLineIndex === 0 || (sdpMLineIndex > 0 &&
	              transceiver.iceTransport !== pc.transceivers[0].iceTransport)) && !maybeAddCandidate(transceiver.iceTransport, cand)) {
	          ...
	          if (candidateString.indexOf('a=') === 0) {
	            candidateString = candidateString.slice(2);
	          }
	          sections = sdp.getMediaSections(pc._remoteDescription.sdp);
	  ...
	  methods.forEach((method) => {
	    ...
	    RTCPeerConnection.prototype[method] = function() {
	      ...
	      if (typeof args[0] === 'function' ||
	        ...
	        return Reflect.apply(nativeMethod, this, [arguments[2]])
	        .then((description) => {
	          if (typeof args[0] === 'function') {
	            Reflect.apply(args[0], null, [description]);
	          }
	        }, (error) => {
	          if (typeof args[1] === 'function') {
	            Reflect.apply(args[1], null, [error]);
	          }
	        });
	      }
	      return Reflect.apply(nativeMethod, this, arguments);
	      ...
	      if (typeof args[1] === 'function' ||
	        ...
	        .then(() => {
	          ...
	          if (typeof args[2] === 'function') {
	            Reflect.apply(args[2], null, [error]);
	          }
	        });
	      }
	      return Reflect.apply(nativeMethod, this, arguments);
	...
	function shimGetUserMedia$2(window) {
	  ...
	  navigator.mediaDevices.getUserMedia = function(c) {
	    return origGetUserMedia(c).catch(error_ => { throw shimError_(error_); });
	  };
	}
	...
	function shimPeerConnection$1(window, browserDetails) {
	  if (window.RTCIceGatherer) {
	    ...
	    if (browserDetails.version < 15_025) {
	  ...
	  if (!(browserDetails.version > 55 &&
	    ...
	    if (MediaStreamTrack && MediaStreamTrack.prototype.getSettings) {
	      ...
	      MediaStreamTrack.prototype.getSettings = function() {
	        const obj = Reflect.apply(nativeGetSettings, this, arguments);
	    ...
	    if (MediaStreamTrack && MediaStreamTrack.prototype.applyConstraints) {
	      ...
	      MediaStreamTrack.prototype.applyConstraints = function(c) {
	        ...
	        return Reflect.apply(nativeApplyConstraints, this, [c]);
	      };
	    }
	  }
	}
	...
	function shimPeerConnection(window, browserDetails) {
	  ...
	  window.RTCPeerConnection.prototype.getStats = function getStats() {
	    const [selector, onSucc, onErr] = arguments;
	    return Reflect.apply(nativeGetStats, this, [selector || null])
	      .then(stats => {
	      ...
	      function addTransceiver() {
	        ...
	        if (shouldPerformCheck) {
	          ...
	          initParameters.sendEncodings.forEach((encodingParam) => {
	            ...
	            if ('scaleResolutionDownBy' in encodingParam && !(Number.parseFloat(encodingParam.scaleResolutionDownBy) >= 1)) {
	                throw new RangeError('scale_resolution_down_by must be >= 1.0');
	              }
	            if ('maxFramerate' in encodingParam && !(Number.parseFloat(encodingParam.maxFramerate) >= 0)) {
	                throw new RangeError('max_framerate must be >= 0.0');
	              }
	          });
	        }
	        const transceiver = Reflect.apply(origAddTransceiver, this, arguments);
	        if (shouldPerformCheck) {
	  ...
	  if (origGetParameters) {
	    window.RTCRtpSender.prototype.getParameters =
	      function getParameters() {
	        const params = Reflect.apply(origGetParameters, this, arguments);
	        if (!('encodings' in params)) {
	          params.encodings = [this.sendEncodings || [{}]].flat();
	        }
	        return params;
	      };
	  }
	...
	function shimCreateOffer(window) {
	  ...
	  window.RTCPeerConnection.prototype.createOffer = function createOffer() {
	    if (this.setParametersPromises && this.setParametersPromises.length > 0) {
	      return Promise.all(this.setParametersPromises)
	      .then(() => {
	        return Reflect.apply(origCreateOffer, this, arguments);
	      })
	      .finally(() => {
	        this.setParametersPromises = [];
	      });
	...
	function shimCreateAnswer(window) {
	  ...
	  window.RTCPeerConnection.prototype.createAnswer = function createAnswer() {
	    if (this.setParametersPromises && this.setParametersPromises.length > 0) {
	      return Promise.all(this.setParametersPromises)
	      .then(() => {
	        return Reflect.apply(origCreateAnswer, this, arguments);
	      })
	      .finally(() => {
	        this.setParametersPromises = [];
	      });
	  ...
	  if (!('addStream' in window.RTCPeerConnection.prototype)) {
	    ...
	    window.RTCPeerConnection.prototype.addTrack =
	      function addTrack(track, ...streams) {
	        ...
	        return Reflect.apply(_addTrack, this, arguments);
	      };
	  }
	...
	function shimCallbacksAPI(window) {
	  ...
	  prototype.createOffer =
	    function createOffer(successCallback, failureCallback) {
	      ...
	      const promise = Reflect.apply(origCreateOffer, this, [options]);
	      if (!failureCallback) {
	        return promise;
	      }
	  ...
	  prototype.createAnswer =
	    function createAnswer(successCallback, failureCallback) {
	      const options = (arguments.length >= 2) ? arguments[2] : arguments[0];
	      const promise = Reflect.apply(origCreateAnswer, this, [options]);
	      if (!failureCallback) {
	        return promise;
	      }
	      promise.then(successCallback, failureCallback);
	  ...
	  let withCallback = function(description, successCallback, failureCallback) {
	    const promise = Reflect.apply(setLocalDescription, this, [description]);
	    if (!failureCallback) {
	      return promise;
	    }
	    promise.then(successCallback, failureCallback);
	  ...
	  withCallback = function(description, successCallback, failureCallback) {
	    const promise = Reflect.apply(setRemoteDescription, this, [description]);
	    if (!failureCallback) {
	      return promise;
	    }
	    promise.then(successCallback, failureCallback);
	  ...
	  withCallback = function(candidate, successCallback, failureCallback) {
	    const promise = Reflect.apply(addIceCandidate, this, [candidate]);
	    if (!failureCallback) {
	      return promise;
	    }
	    promise.then(successCallback, failureCallback);
	...
	function shimRTCIceCandidate(window) {
	  ...
	  window.RTCIceCandidate = function RTCIceCandidate(args) {
	    ...
	    if (args.candidate && args.candidate.length > 0) {
	...
	function shimMaxMessageSize(window, browserDetails) {
	  ...
	  const getCanSendMaxMessageSize = function(remoteIsFirefox) {
	    ...
	    let canSendMaxMessageSize = 65_536;
	    if (browserDetails.browser === 'firefox') {
	      if (browserDetails.version < 57) {
	        if (remoteIsFirefox === -1) {
	          // FF < 57 will send in 16 KiB chunks using the deprecated PPID
	          // fragmentation.
	          canSendMaxMessageSize = 16_384;
	        } else {
	          // However, other FF (and RAWRTC) can reassemble PPID-fragmented
	          // messages. Thus, supporting ~2 GiB when sending.
	          canSendMaxMessageSize = 2_147_483_637;
	        }
	        ...
	        canSendMaxMessageSize =
	          browserDetails.version === 57 ? 65_535 : 65_536;
	      } else {
	  ...
	  const getMaxMessageSize = function(description, remoteIsFirefox) {
	    ...
	    const match = sdp.matchPrefix(description.sdp,
	      'a=max-message-size:');
	    if (match.length > 0) {
	      maxMessageSize = Number.parseInt(match[0].slice(19), 10);
	    } else if (browserDetails.browser === 'firefox' &&
	                remoteIsFirefox !== -1) {
	      // If the maximum message size is not present in the remote SDP and
	      // both local and remote are Firefox, the remote peer can receive
	      // ~2 GiB.
	      maxMessageSize = 2_147_483_637;
	    }
	...
	function shimSendThrowTypeError(window) {
	  ...
	  window.RTCPeerConnection.prototype.createDataChannel =
	    function createDataChannel() {
	      const dataChannel = Reflect.apply(origCreateDataChannel, this, arguments);
	...
	function shimConnectionState(window) {
	  ...
	  ['setLocalDescription', 'setRemoteDescription'].forEach((method) => {
	    ...
	    proto[method] = function() {
	      ...
	      return Reflect.apply(origMethod, this, arguments);
	    };
	  });
	}
	...
	function removeExtmapAllowMixed(window, browserDetails) {
	  ...
	  function setRemoteDescription(desc) {
	    ...
	    return Reflect.apply(nativeSRD, this, arguments);
	  };
	}
	...
	function shimAddIceCandidateNullOrEmpty(window, browserDetails) {
	  ...
	  window.RTCPeerConnection.prototype.addIceCandidate =
	    function addIceCandidate() {
	      ...
	      return Reflect.apply(nativeAddIceCandidate, this, arguments);
	    };
	}
	...
	class AudioTrackConstraints {
	  ...
	  constructor(source) {
	    if (!Object.values(AudioSourceInfo).includes(source)) {
	...
	class VideoTrackConstraints {
	  // eslint-disable-next-line require-jsdoc
	  constructor(source) {
	    if (!Object.values(VideoSourceInfo).includes(source)) {
	      throw new TypeError('Invalid source.');
	    }
	    /**
	     * @member {string} source
	...
	const MediaStreamFactory = {
	  ...
	  createMediaStream(constraints) {
	    ...
	    if (typeof constraints.audio === 'object' && constraints.audio.source === AudioSourceInfo.MIC) {
	      ...
	      mediaConstraints.audio.deviceId = isEdge() ? constraints.audio.deviceId : {
	          exact: constraints.audio.deviceId
	        };
	    } else {
	      mediaConstraints.audio = constraints.audio.source === AudioSourceInfo.SCREENCAST ? true : constraints.audio;
	    }
	    ...
	    return isVideoConstrainsForScreenCast(constraints) ? navigator.mediaDevices.getDisplayMedia(mediaConstraints) : navigator.mediaDevices.getUserMedia(mediaConstraints);
	  },
	};
	...
	function isArrayBufferView(val) {
	  let result;
	  result = (typeof ArrayBuffer !== 'undefined') && (ArrayBuffer.isView) ? ArrayBuffer.isView(val) : (val) && (val.buffer) && (isArrayBuffer(val.buffer));
	  return result;
	}
	 ...
	 */
	function trim(str) {
	  return str.trim ? str.trim() : str.replaceAll(/^\s+|\s+$/g, '');
	}
	  ...
	  function build(data, parentKey) {
	    if (utils.isPlainObject(data) || utils.isArray(data)) {
	      if (stack.includes(data)) {
	        throw new Error(`Circular reference detected in ${  parentKey}`);
	      }
	    ...
	    (function standardBrowserEnv() {
	      return {
	        ...
	        remove: function remove(name) {
	          this.write(name, '', Date.now() - 86_400_000);
	        }
	      };
	    })() :
	...
	const parseHeaders = function parseHeaders(headers) {
	  ...
	  utils.forEach(headers.split('\n'), (line) => {
	    i = line.indexOf(':');
	    key = utils.trim(line.slice(0, Math.max(0, i))).toLowerCase();
	    val = utils.trim(line.slice(i + 1));

	    if (key) {
	      if (parsed[key] && ignoreDuplicateOf.has(key)) {
	        return;
	      }
	      if (key === 'set-cookie') {
	...
	const CanceledError_1 = CanceledError;

	const parseProtocol = function parseProtocol(url) {
	...
	const xhr = function xhrAdapter(config) {
	  return new Promise((resolve, reject) => {
	    ...
	    request.ontimeout = function handleTimeout() {
	      ...
	      const transitional$1 = config.transitional || transitional;
	    ...
	    if (protocol && ![ 'file', 'http', 'https' ].includes(protocol)) {
	...
	const defaults = {
	  ...
	  transformResponse: [function transformResponse(data) {
	    ...
	    if (strictJSONParsing || (forcedJSONParsing && utils.isString(data) && data.length > 0)) {
	      try {
	        return JSON.parse(data);
	      } catch (error_) {
	        if (strictJSONParsing) {
	          if (error_.name === 'SyntaxError') {
	            throw AxiosError_1.from(error_, AxiosError_1.ERR_BAD_RESPONSE, this, null, this.response);
	          }
	          throw error_;
	        }
	      }
	...
	const dispatchRequest = function dispatchRequest(config) {
	  ...
	  return adapter(config).then((response) => {
	    ...
	    if (!isCancel(error_)) {
	      ...
	      // Transform response data
	      if (error_ && error_.response) {
	        error_.response.data = transformData.call(
	          config,
	          error_.response.data,
	          error_.response.headers,
	          config.transformResponse
	        );
	      }
	    }
	...
	const mergeConfig = function mergeConfig(config1, config2) {
	  ...
	  utils.forEach(Object.keys(config1).concat(Object.keys(config2)), (prop) => {
	 ...
	 */
	const isAxiosError = function isAxiosError(payload) {
	...
	const axios$1 = createInstance(defaults_1);
	...
	const es6Promise = createCommonjsModule((module, exports) => {
	...
	const queue = Array.from({length: 1000});
	function flush() {
	...
	const Enumerator = function () {
	  function Enumerator(Constructor, input) {
	    ...
	    if (isArray(input)) {
	      ...
	      this._result = Array.from({length: this.length});
	  ...
	  Enumerator.prototype._eachEntry = function _eachEntry(entry, i) {
	    ...
	    if (resolve$$1 === resolve$1) {
	      ...
	      if (_then === then && entry._state !== PENDING) {
	        ...
	        this._willSettleAt(new c((resolve$$1) => {
	...
	function polyfill() {
	  ...
	  if (commonjsGlobal !== undefined) {
	    ...
	    try {
	      ...
	      throw new Error('polyfill failed because global object is unavailable in this environment', { cause: error_ });
	    }
	  } else {
	    local = self;
	  }
	...
	class RTCEndpoint extends Event$1 {
	  ...
	  constructor(options) {
	    ...
	    if (this.options.usedatachannel) {
	      ...
	      this.datachannel.addEventListener('close', this.e.ondatachannelclose);
	      this.datachannel.onerror = this.e.ondatachannelerr;
	      this.datachannel.onmessage = this.e.ondatachannelmsg;
	      this.datachannel.addEventListener('open', this.e.ondatachannelopen);
	    }
	  }
	  ...
	  // 新增逻辑添加Dtmf
		_onRemoteStream(event) {
			log(this.TAG, 'Received remote stream', event);
			if (!this.pc.getSenders) {
				log(this.TAG, 'This demo requires the RTCPeerConnection method getSenders() which is not support by this browser.', event);
				return;
			}
			const senders = this.pc.getSenders();
			log(this.TAG, ' senders :', event, senders);
			const audioSender = senders.find(sender => sender.track && sender.track.kind === 'audio');
			if (!audioSender) {
				log(this.TAG, 'No local audio track to send DTMF with\n', event);
				return;
			}
			if (!audioSender.dtmf) {
				log(this.TAG, 'This demo requires DTMF which is not support by this browser.', event);
				return;
			}
	  ...
	  _sendSdp(){
	    ...
	    this.pc.onicecandidateerror = null;
			if(this.options.recvOnly){
				...
				this.options.funApi(sdp).catch(error_ => {
					error(this.TAG, error_);
				...
				this.options.funApi(sdp).then(response => {
					const anwser = { sdp: response?.sdp, type: 'answer',};
	  ...
	  sendTones(tones,duration = 200,gap = 50) {
			if (this.dtmfSender && ((this.dtmfSender.canInsertDTMF) === undefined || this.dtmfSender.canInsertDTMF)) {
				log(this.TAG, 'Tones, duration, gap: ', tones, duration, gap);
	  ...
	  return new Promise((resolve, reject) => {
	    if (typeof navigator.mediaDevices !== 'object' || typeof navigator.mediaDevices.enumerateDevices !== 'function') {
	      ...
	      navigator.mediaDevices.enumerateDevices().then(devices => {
	        ...
	        reject(`${error_.name}: ${error_.message}`);
	      });
	    }
	  });
	}
