<template>
  <div>
    ...
    <div :class="`${prefixCls}-left bg-black grid row-span-15`">
      <FsRtcPlay ref="fsRtcPlay" :sendSdpApi="sendSdpApi" :muted="false" />
    </div>
...
<script lang="ts" setup>
    ...
    return new Promise((resolve, reject) => {
      if (stats.isCall) {
        ...
        useSocket.sendMessage(
          SocketNamespace.AGENT_NAMESPACE,
          SocketInEvent.AGENT_IN_CALL_NOTIFICATION,
          {
            type: 2,
            callId: stats.callId,
            sdp: sdp,
  ...
  onBeforeMount(() => {
    ...
    rootSocketEmitter.on(SocketOutEvent.AGENT_OUT_STATUS, (val) => {
      ...
      const { agentState, agentName, agentKey, agentId, mediaAddress, loginType } =
        data as Recordable;
