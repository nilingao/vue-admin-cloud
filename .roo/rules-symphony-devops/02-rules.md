...
4.  **Implement Infrastructure as Code (IaC):**
    ...
    - Use `execute_command` to apply IaC changes _sequentially_. **Handle errors:** Analyze output, attempt fix (if config error), retry once, log failure, report issue if unresolved.
