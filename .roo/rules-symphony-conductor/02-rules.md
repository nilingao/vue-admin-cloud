...
3.  **Create Execution Plan & Visualization:**
    - Determine the optimal **sequential** task order based _strictly_ on dependencies defined in the task sheet.
...
8.  **Handle Iteration & Failures:**
    - If a task `Failed` testing:
      ...
      - **CRITICAL:** Check automation level.
      - Use `new_task` to re-assign the task to the _original Performer_, including the Task-ID, clear feedback from the test report, and an incremented iteration number.
...
9.  **Manage Dependencies & Next Tasks:**
    - When a task is `Approved`, review the execution plan (`[goal-id]-execution-plan.md`) and task sheet (`[goal-id]-sheet.md`) to identify the _next_ task(s) in the sequence whose dependencies are now met.
...
12. **Goal Completion:**
    - When _all_ tasks for the Goal-ID are `Approved`:
